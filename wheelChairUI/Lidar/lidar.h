...
struct LidarData {
    ...
    double angle;
    double distance;
    bool isOk() const { return distance > 0 && quality > 5; }
};
...
public:
    Lidar(std::string portname, std::function<void(bool startSweep, const LidarData& point)> handler, bool autoStartScan = true);
    virtual ~Lidar();
public:
