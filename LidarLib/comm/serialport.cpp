...
{
    return static_cast<int>(sp_input_waiting(port)) > 0;
}

char SerialPort::readChar()
{
    char c = 0;
    sp_blocking_read_next(port, &c, 1, 0);
    return c;
}
...
{
    throwOnError(sp_nonblocking_write(port, data, length));
}
...
{
    return sp_output_waiting(port) > 0;
}
